    ...
    def guess(self, guess_history):
        ...
        for i in range(len(self.words)):
            ...
            for option in possible_options:
                ...
                if outcomes[outcome_id] > best_worst_outcome:                                   # se outcome best worst outcome è monore prende un altra parola del primo ciclo
                    break
            ...
            if worst_outcome < best_worst_outcome or (worst_outcome == best_worst_outcome and (
                    best_word not in possible_options or self.word_probability.is_wordle_probability(
    ...
    for element in guess_history:  # guess_history is a list of list were the first element =[0] of each sublist is the guesses word and the second element =[1] is a list containing the LetterPositionInformation for each letter in the word
        for i in range(WORD_LENGTH):
            if element[1][i] == LetterInformation.CORRECT:
                ...
                correct_letter_position.add((element[0][i],
                                             i))  # adds to the set of correct letters the letter and its corresponding correct position
...
def calculate_outcome(word, option):                #method to create an id for each word
    outcome = 0
    for i in range(WORD_LENGTH):
        if word[i] == option[i]:
            outcome += 2 * 3 ** i
        elif word[i] not in option:
            outcome += 3 ** i
