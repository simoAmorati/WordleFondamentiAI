...
class QuordleTester:
    ...
    def __init__(self, wordlist_filename="combined_wordlist.txt"):
        self.wordlist = WordList(wordlist_filename)
        self.words = self.wordlist.get_list_copy()
        self.competitor = QuordleAI(self.words)

    def fight(self, rounds, solution_wordlist_filename="shuffled_real_wordles.txt"):
    ...
    def play(self, competitor, word):
        ...
        for i in range(MAX_GUESSES):  # Up to 9 guesses
            ...
            guess_result = [[] for _ in range(GAMES) ]
...
def main():
    ...
    competition = QuordleTester(wordlist_filename="combined_wordlist.txt")
    competition.fight(rounds=1000, solution_wordlist_filename="shuffled_real_wordles.txt")
